...
export default function WorldviewPage() {
  const [responses, setResponses] = useState<{ [key: string]: number | null }>(
    Object.fromEntries(worldviewItems.map((item) => [item, null]))
  );
  ...
  useEffect(() => {
    ...
    async function fetchUserData() {
      try {
        ...
        // ✅ Use updated boolean field from backend
        if (data.has_filled_questionnaire) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:5000/questionnaire", {
        ...
        body: JSON.stringify({
          username: username,
          worldview_questionnaire: responses,
        }),
      });

      if (!response.ok) {
        throw new Error("Failed to submit worldview questionnaire");
      }

      console.log("Worldview questionnaire submitted:", responses);
      router.push("/");
    } catch (err) {
  ...
  if (alreadySubmitted) {
    return (
      <Box sx={{ maxWidth: 700, mx: "auto", p: 3, textAlign: "center" }}>
        <Typography variant="h5" gutterBottom>
          You already filled out the worldview questionnaire.
        </Typography>
